<div class="wrapper">  
  ...
  <div class="login_detail right">
    ...
    <form id="AssessmentForm" name="AssessmentForm" method="post" action="<?php echo $this->webroot; ?>assessment/project_management_edit_action" class="form">
    <fieldset>    	
    	<?php $i=0; $p=0;
		//pr($responses);
			 foreach($data as $dat){ 
        ...
        <div class="inner none"> 
                <table class="table none">
				  ...
				  foreach($dat['question'] as $ques){ ?>
                    ...
                    <input type="hidden" name="data[<?php echo $i; ?>][<?php echo $j; ?>][id]" value="<?php echo $responses[$p]['Response']['id']; ?>"/>
                    <?php for($m=5;$m>0;$m--){ ?>
                    <td align="center"><div class="select"><input class="check validate[required]" type="radio" name="data[<?php echo $i; ?>][<?php echo $j; ?>][check]" value="<?php echo $m; ?>" onClick="$('.tooltip').hide(); $('#tooltip_<?php echo $i.'_'.$j.'_'.$m; ?>').fadeIn('fast');" <?php if(isset($is_response)){ if(($m==$responses[$p]['Response']['response'])) echo 'checked="checked"'; } ?>>
                    <div class="tooltip" id="tooltip_<?php echo $i.'_'.$j.'_'.$m; ?>">
            					<div class="tt"><?php echo __('Please enter comments if you wish to support your scoring'); ?>
                            		<span class="input_box"><input type="text" name="data[<?php echo $i; ?>][<?php echo $j; ?>][inp][<?php echo $m; ?>]" <?php if($m==$responses[$p]['Response']['response']){ ?> value="<?php echo $responses[$p]['Response']['comments']; ?>" <?php }else{ ?> placeholder="Comments" <?php } ?> /><a href="javascript://" onclick="$('.tooltip').fadeOut('fast');"><img src="<?php echo $this->webroot; ?>img/yes.png" alt=""/></a></span>
                            	</div>
            					<div class="arrow"> </div>
            				</div>
