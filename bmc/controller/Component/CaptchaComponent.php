...
class CaptchaComponent extends Object
{
	...
	function create($width='120',$height='40',$characters='6') {
		...
		/* output captcha image to browser */
				$this->Controller->Session->write('Config.security_code',$code);

		header('Content-Type: image/jpeg');
		imagejpeg($image);
		imagedestroy($image);
		//$this->Controller->Session->write('Config.code','1');

	}

	function getVerCode()	{
		//print_r($this->Controller->Session->read());die;
		return $this->Controller->Session->read('Config.security_code');
	}
}
?>