<?php 
//	echo '<pre>'; print_r($this->element('sql_dump')); die;
   if(count($searchData)>0){
	...
	?>  
        <div id="filterCount"><?php echo $totalFilter.' Profiles';?></div>
...
//								   echo '<span>'; 	 
							       if(isset($profList['Professional']['joining_by_date'])) {
									   ...
									   if($date[0] != '0000-00-00')  {
    									  ...
    									  echo '<span title="Available from '.$format[2].'-'.$format[1].'-'.$format[0].'">'.$format[2].'-'.$format[1].'-'.$format[0].'</span>'; 
									   }
								   }
							       elseif(isset($profList['Professional']['joining_by_day'])) {
									    echo '<span title="Available in '.trim($profList['Professional']['joining_by_day']).' days">'.trim($profList['Professional']['joining_by_day']).' days'.'</span>'; 
								   }
						           elseif($profList['Professional']['immediate_joining_flag'] == 1) {
								    echo '<span title="Available immediately">Immediate</span>'; 
							       }
//								   echo '</span>';
								 }
                        ...
                        <section class="detail"><span>
                            ...
                            <?php 
								...
								$in=0;
								foreach($arrPrefLocation as $ploc){$in++;
									$ploc=trim($ploc);
									if($ploc!=''){
										if(count($arrPrefLocation)>$in){
											echo "<span>{$ploc}&nbsp; </span>";
										}else{
											echo "<span>{$ploc}</span>";
										}
		...
		$this->Paginator->options(array(
						 'update' => '.candidate_details',
					     'url'=>array('controller'=>'recruiters','action'=>'professional_search'),
						 'evalScripts' => true,
//					     'before' => $this->Js->get('#indiactor')->effect('fadeIn',array('buffer' => false)),
//						 'complete' => $this->Js->get('#indiactor')->effect('fadeOut',array('buffer' => false)),
                         'data'=>http_build_query($formData),
						 'method'=>'POST'
		));

		if($this->Paginator->counter('{:count}')>$listRange) {
?>	
           <div class="paginator">
