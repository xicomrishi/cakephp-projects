...
class RechargeController extends AppController {
	...
	public $uses=array('User','Operator','Recharge','GiftCategory','GiftBrand','RechargeLog','Circle','VoucherLog','Denomination','Config');
	...
	{		
		...
		if($num==1)
		{
			...
			$data['Recharge']['recharge_value']=number_format($data['Recharge']['recharge_value'],2);
			$recharge_value=number_format($data['Recharge']['recharge_value'],2);
					
			$session_id=String::uuid();
			$arr=array('number'=>$data['Recharge']['number'],'recharge_type'=>$data['Recharge']['recharge_type'],'voucher_code'=>$data['Recharge']['voucher_code'],'amount'=>$recharge_value,'voucher_amount'=>$data['Recharge']['amount'],'operator_id'=>$data['Recharge']['operator_id'],'circle_id'=>$data['Recharge']['circle_id'],'session_id'=>$session_id,'created'=>date("Y-m-d H:i:s"));
	...
	public function validate_voucher_code($num=0)
	{
			...
			if(isset($response->vQueryVoucherResult[0]))
			{
				...
				if(trim(strtolower($arr->ResultType))=="success")
				{
					...
					{ 
						$config=$this->Config->find('first',array('conditions'=>array('Config.voucher_value'=>intval(trim($arr->Value)))));
						//pr($config); die;
						if(!empty($config))
						{
							$usablevalue=$config['Config']['display_value'];
						}else{							
							$usablevalue=$arr->Value;
						}
						//$usablevalue=$arr->Value;
						$msg = 'success|'.$usablevalue.'|'.$arr->VoucherNumber;	
	...
	{		
		...
		if($this->request->is("post") && isset($this->request->data)){			
			...
			if(intval($split[1]) < intval($this->request->data['Recharge']['amount']))
			{
				$this->request->data['Recharge']['amount']==number_format($split[1],2);	
			}
	...
	public function back_to_index()
	{
		...
		$this->set('rel_circles',$rel_circles);	
		$this->set('circles',$circles);
		$rechargeLog=$this->RechargeLog->find('first',array('conditions'=>array('RechargeLog.id'=>$this->data['Recharge']['log_id'])));
		$this->set('rechargeLog',$rechargeLog);
		...
		if(!$this->Session->check('RecAdmin'))
		{
			$this->redirect(array('controller'=>'login','action'=>'index','admin'=>true));	
				...
				$this->Recharge->bindModel(array('belongsTo'=>array('User'=>array('className'=>'User','foreignKey'=>'user_id'),
																	...
																	'Circle'=>array('className'=>'Circle','foreignKey'=>'circle_id')
																	)));
	...
	{		
			...
			$data[0][6]='Voucher';
			$data[0][7]='Amount';
			$data[0][8]='Recharge Status';
			$data[0][9]='Recharge Verif.';
			$data[0][10]='Date';
			...
			{				
				...
				if(!empty($rech['Circle']['circle'])) $circle=$rech['Circle']['circle']; else $circle='N/A';
				...
				$data[$i][5]=$circle;
				$data[$i][6]=$rech['Recharge']['voucher_code'];
				$data[$i][7]=$rech['Recharge']['amount_from_voucher'];
				$data[$i][8]=$re_pay_status;
				$data[$i][9]=$re_status;
				$data[$i][10]=show_formatted_datetime($rech['Recharge']['recharge_date']);
			...
			$objPHPExcel->setActiveSheetIndex(0)->getStyle("A1:K1")->applyFromArray(array("font" => array( "bold" => true)));
	...
	public function temp()
	{
		...
		foreach($circles as $cr)
		{
			$allc[]=strtolower($cr['Circle']['circle']);
			
		}	
		$inputs=array();
		foreach($inputs as $inp)
		{
			if(!in_array($inp,$allc))
			{
				$insert[]=$inp;	
			}	
		}
