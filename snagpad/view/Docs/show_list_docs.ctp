...
<div id="success_m" style="display:none;"></div>
...
<section class="contact_section">
        ...
        <span class="col1"><input type="checkbox" id="all_check" onclick="select_all_check();"/></span>
        <span class="col2">FILE</span>
        <span class="col3">shared</span>
        <span class="col4">UPLOADED</span>
        ...
        <span class="col1"><input type="checkbox" name="cbox[]" class="doc_check" onclick="objDelChecked(this);uncheck();" value="<?php echo $file['File']['id']; ?>"></span>
        <span class="col2"><a href="javascript://" onclick="show_edit(<?php echo $file['File']['id']; ?>);"><?php echo $file['File']['filename']; ?></a></span>
        <span class="col3"><input type="checkbox" <?php if($file['File']['shared']=='Y') { echo 'checked'; } ?> disabled="disabled"></span>
...
$(document).ready(function(e) {
	...
	setTimeout(function(){$('#success_m').hide()},3000);
...
function ConfirmChoice()
{
	...
	{
		alert("Please select the document(s) which you want to delete.");
		return false;
	}
	else
	{
		answer = confirm("Are you sure you want to delete the selected document(s)?");
		if (answer !=0)
		{
			return true;
		}
...
function select_all_check()
{
	...
	{
		$('.doc_check').each(function(index, element) {
