...
function submit_textboxtype()
{
	...
	if(flag=='1'){
			...
			if(text=='')
			{
				...
				$.post('<?php echo SITE_URL;?>/strategies/<?php echo $action;?>',$('#strat_textboxForm').serialize(),function(data) {
					disablePopup();
		...
		$('#strat_textboxForm').validate({
			submitHandler: function(form){
					...
					if(f1==''&&f2==''&&f3==''&&f4=='')
					{
						...
						$.post('<?php echo SITE_URL;?>/strategies/<?php echo $action;?>',$('#strat_textboxForm').serialize(),function(frm1) {
							disablePopup();
							$('#li_a_'+check_id).addClass('done');
							get_strategy_meter();
							get_bar_meter_percent();
