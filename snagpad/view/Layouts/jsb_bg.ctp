...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <?php
        echo $this->Html->script(array('jquery', 'jquery.validate2.min','popup','datepicker/jquery.ui.core.min','fileupload/jquery.ui.widget','datepicker/jquery.ui.datepicker.min','PIE','tooltip'));?>
...
<?php if(in_array($this->Session->read('Client.Client.id'), array(6,33,1,697))){ ?>
<?php if($this->Session->read('usertype')==3){?> <a href="javascript://" class="challenge_btn" onclick="show_challenge_scroll();">challenge</a><?php } else if($this->Session->read('usertype')==2){?><a href="javascript://" class="challenge_btn" onclick="show_challenge_scroll();">challenge</a>
<?php } } else { ?> <a href="#" class="challenge_btn">challenge <span class="coming_tip">COMING SOON</span></a>
<?php }?>
...
<div class="challenge_detail">
    ...
    <a href="<?php echo SITE_URL; ?>/challenges/index/ch_progress" class="pad">Challenges in Progress</a>
    <a href="<?php echo SITE_URL; ?>/challenges/index" class="pad">View all Challenges</a>
    <a href="<?php echo SITE_URL; ?>/challenges/index/start" class="pad" id="start_ch" style="display:none;">Start challenge now</a>
</div>
</div>
	...
	if(show_challenge==0)
	{
		...
		$.post('<?php echo SITE_URL; ?>/challenges/challenge_points/'+clientid,{cl_id:clientid},function(data){
				...
				if(rep[3]==1)
					...
					$('#start_ch').hide();
				$('.challenge_detail').slideDown(2000);	
