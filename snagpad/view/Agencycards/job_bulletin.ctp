<section id="inner_body_container">
    <section class="tabing_container spacer0">
      <section class="tabing" style="margin:-64px 0 0 0">
        <ul>
          <li class="active spacer"><a href="#" class="border">SEARCH FOR JOBS</a></li>
        </ul>
      </section>
...
function searchJob()
{
		...
		$.post("<?php echo SITE_URL; ?>/agencycards/job_bulletin_mid",frmval,function(data){	
		if(data=='Error')
...
function showPage(i)
{
 ...
 function add_card(id)
    {
            $.post('<?php echo SITE_URL; ?>/agencycards/add_card/'+id,'',function(data){ 
			$('#error').html(data);
