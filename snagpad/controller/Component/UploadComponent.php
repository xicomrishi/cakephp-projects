...
class UploadComponent extends Component
{
    ...
    function __construct( ComponentCollection $collection, $options = null ) {
        ...
        $this->options = array(
            ...
            'max_file_size' => null,
            'min_file_size' => 1,
            'accept_file_types' => '([^\s]+(\.(?i)(docx|doc|pdf|txt))$)', // For only accept images use this: ([^\s]+(\.(?i)(jpg|png|gif|bmp))$)
    ...
    protected function set_file_delete_url($file) {
		if(isset($file->mod_name)){ $f_name=$file->mod_name;}else{ $f_name=$file->name;}
        $file->delete_url = $this->options['script_url']
            .'?file='.rawurlencode($f_name);
    ...
    public function delete() {
       ...
       // $file_path = $this->options['upload_dir'].$file_name;
        ...
        if ($success) {
            ...
            $aux = $this->UploadModel->findByFile( $file_name );
			//echo '<pre>';print_r($aux); die;
            $this->UploadModel->delete( $aux['Clientfile']['id'] );
