 ...
 */
 public function upload($file, $destination_dir, $user_thumb = false, $save_filename = '', $params = array())
 {
	if ( !empty($file) )
	{		
		...
		if ( in_array($ext, $this->allow_extensions ) && $file['error'] == 0 )
		{
			...
			if ( $file_info['0'] < 155 || $file_info['1'] < 155)
			{
				$this->error = true;
				$this->errorMessage =  __('Image width and height should not be less than 160px');
				return false;
			}
 ...
 */
	public function crop( $filedata, $file_dimension, $is_thumb = true, $thumbs = array() )
	{
