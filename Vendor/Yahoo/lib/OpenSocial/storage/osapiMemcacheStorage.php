 ...
 * @author Chris Chabot
 */
class osapiMemcacheStorage extends osapiStorage {
  private $connection = false;

  public function __construct($host, $port) {
    if (! function_exists('memcache_connect')) {
      throw new osapiStorageException("Memcache functions not available");
    }
    if ($host == '' || $port == '') {
      throw new osapiStorageException("You need to supply a valid memcache host and port");
    }
    $this->host = $host;
    $this->port = $port;
  }
  ...
  public function set($key, $value) {
    ...
    if (@memcache_set($this->connection, $key, array('time' => time(), 
      ...
      throw new osapiStorageException("Couldn't store data in cache");
    }
  }
